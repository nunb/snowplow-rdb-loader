...
object RedshiftLoadStatements {
  ...
  def buildLoadStatements(
    ...
    RedshiftLoadStatements(target.schema, atomicCopy, shreddedCopyStatements, vacuum, analyze, manifestStatement, base)
  }
  ...
  def buildEventsCopy(config: SnowplowConfig, target: RedshiftConfig, s3path: S3.Folder, transitCopy: Boolean): AtomicCopy = {
    ...
    if (transitCopy) {
      TransitCopy(SqlString.unsafeCoerce(s"""
        |COPY ${Common.getTable(target.schema, Common.TransitEventsTable)} FROM '$s3path'
      ...
      StraightCopy(SqlString.unsafeCoerce(s"""
  ...
  def buildAppendStatement(schema: String, targetTable: String, sourceTable: String): SqlString =
    SqlString.unsafeCoerce(s"ALTER TABLE ${Common.getTable(schema, targetTable)} APPEND FROM $sourceTable")
